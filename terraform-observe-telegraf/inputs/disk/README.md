...
This module registers metrics collected by the disk [input plugin](https://github.com/influxdata/telegraf/blob/master/plugins/inputs/disk/README.md)
...
module "telegraf_disk" {
  source       = "git@github.com:observeinc/terraform-observe-telegraf.git//inputs/disk"
  workspace    = data.observe_workspace.default
  ...
  link_targets = {
    ...
    "volume": { 
      "target": module.telegraf.volume.oid, 
...
| observe_dataset.disk_metrics | resource |

| observe_link.disk_metrics | resource |
...
| Name | Description |
|------|-------------|
| <a name="output_disk_metrics"></a> [disk\_metrics](#output\_disk\_metrics) | n/a |
